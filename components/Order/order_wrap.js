    ...
    return (
      <section className={styles.dashboard_wrap}>
        ...
        <div className="flex min-h-[calc(max(42rem,100vh)-4rem)]">
          <div
            className={`min-h-full transition duration-75  w-[25rem] ${
              minimize ? "" : "translate-x-[-85%]"
            }`}>
          ...
          <div
            className={`float-righ max-h-[100vh] transition duration-100 ${
              minimize ? "w-[calc(100vw)]" : "-ml-[21rem] w-[calc(100vw)]"
            }`}>
            {children}
          </div>
        </div>
